...
static std::vector<std::function<int(int,int, int)>> cmd_msg;
std::map<int, std::function<int(int,int, int)>> cmdFuncs;
void AddMsgFunc(std::function<int(int, int)> f) {
	msg.push_back(f);
}

void AddMsgFunc(std::function<int(int,int, int)> f) {
	cmd_msg.push_back(f);
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	for (std::map<int, std::function<int(int,int, int)>>::iterator itor = cmdFuncs.begin();  itor !=cmdFuncs.end(); itor++) {
		int k =itor->first;
		if (message == k) {
			itor->second((int)hWnd,wParam, lParam);
		}
	}
